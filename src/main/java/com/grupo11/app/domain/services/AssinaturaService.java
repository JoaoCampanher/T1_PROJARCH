...
public boolean validaAssinatura(UUID codass){
        Assinatura assinatura = assinaturaRepositorio.findById(codass).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Assinatura não encontrada"));
        return assinatura.getStatusAssinatura().equals(Status.ATIVA);
    }
